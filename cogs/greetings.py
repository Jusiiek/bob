    ...
    async def on_submit(self, interaction: Interaction):
        try:
            self.config.set_config(
                ConfigKeys.WELCOME_CHANNEL_TEXT.value,
                self.text.value
            )
...
class Greetings(Base):
    ...
    async def set_welcomes_channel(self, interaction: Interaction, channel: discord.TextChannel):
        ...
        self.__config.set_config(
            ConfigKeys.WELCOME_CHANNEL.value,
            channel.id
        )

    @commands.Cog.listener(name='on_member_join')
    async def on_member_join(self, member: discord.Member):
        channel_id = Config.get_config_value(ConfigKeys.WELCOME_CHANNEL.value)
        if channel_id:
            guild = self.__bot.get_guild(SERVER_ID)
            channel = self.__bot.get_channel(Config.get_config_value(ConfigKeys.WELCOME_CHANNEL.value))
            if channel is not None:
                await channel.send(
                    f'Hey {member.mention}, welcome to {guild.name}! :partying_face:\n' +
                    f"{self.__bot.get_channel(Config.get_config_value(ConfigKeys.WELCOME_CHANNEL_TEXT.value))}"
                )
